...
export const comparePassword = async (plainText: string, storedPassword: string): Promise<boolean> => {
  ...
  // Se não for bcrypt, pode ser formato de criptografia antiga ou plaintext
  // Tentar descriptografar e comparar
  try {
    const decrypted = decryptPassword(storedPassword)
    return decrypted === plainText
  } catch {
    // Se não conseguir descriptografar, comparar diretamente (fallback para app-erp plaintext ou logs)
    return storedPassword === plainText
  }
}

// Funções de criptografia simétrica (legado AES)
